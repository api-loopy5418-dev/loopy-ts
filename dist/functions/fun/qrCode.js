...
async function qrCode(text) {
    if (text === undefined)
        throw new e.MissingArgsError("LoopyError: qrCode expected a string for text");
    ...
    try {
        return await axios_1.default.get(`https://api.loopy5418.dev/qr?data=${encodeURIComponent(text)}&key=${key}`, {
            timeout: 3000,
        });
    }
    catch (err) {
