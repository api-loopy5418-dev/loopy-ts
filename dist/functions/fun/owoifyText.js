...
async function owoifyText(text) {
    if (text === undefined)
        throw new e.MissingArgsError("LoopyError: owoifyText expected a text.");
    try {
        return await axios_1.default.get(`https://api.loopy5418.dev/owoify?text=${encodeURIComponent(text)}`, {
            timeout: 3000,
        });
    }
    catch (err) {
