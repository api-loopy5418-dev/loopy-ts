...
export async function currencyConveter(base: string, target: string, amount: number) {
  ...
  return await axios.get(`https://api.loopy5418.dev/currency-converter?base=${base}&target=${target}&amount=${money}`)
}
