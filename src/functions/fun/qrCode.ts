...
export async function aiGenerate(text: string) {
  ...
  const qr: string = encodeURIComponent(prompt)
  return await axios.get(`https://api.loopy5418.dev/qr?data=${gr}&key=${key}`)
}
